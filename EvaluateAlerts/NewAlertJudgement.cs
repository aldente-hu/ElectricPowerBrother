...
namespace HirosakiUniversity.Aldente.ElectricPowerBrother
{
	...
	namespace EvaluateAlerts
	{
		namespace New
		{
			...
			public class AlertJudgement : Helpers.New.IUpdatingPlugin
			{
				...
				public AlertJudgement(Data.IConnectionProfile profile) : this()
				{
				...
				public async Task JudgeAsync()
				{
					...
					var recent_data = (await c_data.GetRecentDataAsync(3)).OrderByDescending(d => d.Key);
					var current_rank = await data.GetCurrentRankAsync();

					int new_rank = Check(current_rank, recent_data.Select(d => d.Value).ToArray());
					if (new_rank != current_rank)
					{
						// 記録する．
						await data.InsertDataAsync(new AlertElement
						{
							DataTime = recent_data.First().Key,
				...
				public async Task OutputAtomFeedAsync(string destination, int n = 6)
				{
					...
					// 6件取得．
					var recent_data = await data.GetRecentDataAsync(n);
					...
					for (int i = 0; i < times.Length - 1; i++)
					{
						...
						{
							ID = this.FeedID + Data.DataModelBase.TimeConverter.TimeToInt(current.DeclaredAt),
				...
				public Helpers.New.Jappajil MailSender
				{ get; set; }
				...
				private async Task SendMailAsync(bool separate_destinations)
				{
					...
					// message本文を作成する．
					var recent_data = await data.GetRecentDataAsync(2);
					var times = recent_data.Keys.OrderByDescending(k => k).ToArray();
					...
					if (separate_destinations)
					{
						foreach (var to in MailDestinations)
						{
							message.To.Clear();
							message.To.Add(to);
							await this.MailSender.PostAsync(message);
				...
				{
					DateTime latestDataTime = await c_data.GetLatestDataTimeAsync();
						...
						switch (element.Name.LocalName)
						{
							...
							case "Atom":
								foreach (var attribute in element.Attributes())
								{
									...
									{
										case "Destination":
											this.Inserted += async (sender, e) => { await this.OutputAtomFeedAsync(attribute.Value); };
											break;
										case "Id":
							...
							case "Mail":
								...
								MailSender = new Helpers.New.Jappajil(element.Attribute("SmtpServer").Value, port ?? 25);
								...
								this.Inserted += async (sender, e) => { await this.SendMailAsync(true);  /* 引数は決め打ち． */
								 };
