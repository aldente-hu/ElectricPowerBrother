	...
	namespace New
	{
		...
		public class Environment
		{
			...
			*/
			...
			public Environment(Data.IConnectionProfile profile, System.Xml.Linq.XElement loggers)
			{
			...
			public async Task<DateTime> GetNextDataTimeAsync()
			{
				return (await db.GetLatestDataTimeAsync()).AddMinutes(10);
			}
			...
			public async Task RunAsync(bool saving = false)
			{
				DateTime next_data_time = (await db.GetLatestDataTimeAsync()).AddMinutes(10);
			...
			public async Task ProcessDataAsync(IEnumerable<IDictionary<DateTime, TimeSeriesDataDouble>> results, DateTime next_data_time, bool saving = false)
			{
				while (results.All(result => { return result.Keys.Contains(next_data_time); }))
				{
					...
					if (saving)
					{
						await db.InsertDataAsync(next_data_time, sum.Data);
