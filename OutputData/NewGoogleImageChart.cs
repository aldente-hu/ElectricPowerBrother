...
namespace HirosakiUniversity.Aldente.ElectricPowerBrother
{
	...
	namespace New
	{
		...
		{
			public GoogleImageChart(Data.IConnectionProfile profile) : base(profile) { }

			#region *グラフを出力(DrawChart)
			public async Task DrawChartAsync(DateTime dataTime)
			{
				...
				if (DataCh.HasValue)
				{
					query["chd"] = "t:" + string.Join(",", await GetDataArrayAsync(dataTime, DataCh.Value));   // データ
				}
				else
				{
					query["chd"] = "t:" + string.Join(",", await GetDataArrayAsync(dataTime));   // データ
				}
			...
			async Task<ICollection<int>> GetDataArrayAsync(DateTime latestTime)
			{
				// 100分率データを返す．
				return (await this.GetDetailConsumptionsAsync(latestTime.AddHours(-36), latestTime)).OrderBy(pair => pair.Key).Select(pair => ConvertYData(pair.Value)).ToArray();
			}

			async Task<ICollection<int>> GetDataArrayAsync(DateTime latestTime, int ch)
			{
				return (await this.GetParticularConsumptionsAsync(latestTime.AddHours(-36), latestTime)).OrderBy(pair => pair.Key).Select(pair => ConvertYData(pair.Value[ch])).ToArray();
			}
			...
			public void Configure(System.Xml.Linq.XElement config)
			{
				...
				this.UpdateAction = async (time) => { await DrawChartAsync(time); };
			}
			#endregion
