...
namespace HirosakiUniversity.Aldente.ElectricPowerBrother
{
	...
	namespace New
	{
		...
		/// </summary>
		public class GnuplotTrinityChart : Helpers.New.PltFileGeneratorBase, IUpdatingPlugin
		{
			#region プロパティ
			...
			#region *Pltファイルを生成(GenerateAsync)
			public override async Task GenerateAsync(StreamWriter writer, DateTime time)
			{
				// timeは未使用．
				...
				{
					await writer.WriteLineAsync($"cd '{RootPath}'");
				}
				await writer.WriteLineAsync($"set terminal svg enhanced size {this.Width},{this.Height} fsize {this.FontSize}");
				await writer.WriteLineAsync($"set output '{ChartDestination}'");  // ※とりあえず決め打ち．
				await writer.WriteLineAsync("set encoding utf8");
				await writer.WriteLineAsync("set title \"電力消費量 (理工学部＋α)\""); // 出力フォーマットによっては全角の"＋"は使わない方がよい．

				// 各軸の設定
				await writer.WriteLineAsync("set xlabel '時刻 [Hour]'");
				await writer.WriteLineAsync("set xrange [ 0.0 : 24.0 ]");
				await writer.WriteLineAsync("set xtics border mirror norotate 0,1,24");

				await writer.WriteLineAsync("set ylabel '10分間電力消費量 [kWh]'");
				await writer.WriteLineAsync(string.Format("set yrange [ {0} : {1} ]", bottom_power, top_power));
				await writer.WriteLineAsync(string.Format("set ytics border {0},10,{1}", bottom_power, top_power));

				await writer.WriteLineAsync("set y2label '気温 [℃]'");
				await writer.WriteLineAsync(string.Format("set y2range [ {0} : {1} ]", min_temp, max_temp));
				await writer.WriteLineAsync(string.Format("set y2tics border {0},{2},{1}", min_temp, max_temp, step_temp));

				// レイアウトの設定
				await writer.WriteLineAsync("set grid y2tics");
				await writer.WriteLineAsync("set key left top autotitle columnheader");  // これを使うと文字エンコーディングがおかしくなる？←場合がある．
	...
	set output
			...
			public GnuplotTrinityChart(Data.IConnectionProfile profile)
				: base()
			{ }
			...
			public async Task UpdateAsync()
			{
				...
				{
					await Gnuplot.GenerateChartAsync(this, _current);
				}
				_current = latestData;
			}
			DateTime _current;

			#region *プラグインを設定(Configure)
