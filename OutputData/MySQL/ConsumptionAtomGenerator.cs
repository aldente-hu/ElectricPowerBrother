...
namespace HirosakiUniversity.Aldente.ElectricPowerBrother.MySQL
{
	...
	public class ConsumptionAtomGenerator : ConsumptionData, IPlugin
	{
		...
		public ConsumptionAtomGenerator(ConnectionProfile profile) : base(profile) {	}
		#endregion
		...
		public IDictionary<int, int> GetConsumptionsOn(DateTime time)
		{
			...
			using (var connection = new MySql.Data.MySqlClient.MySqlConnection(Profile.ConnectionString))
			{
				connection.Open();
				using (var command = connection.CreateCommand())
				{
					...
					command.Parameters.Add(new MySqlParameter("@1", TimeConverter.TimeToInt(time)));
