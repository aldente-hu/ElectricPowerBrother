		...
		namespace New
		{
			...
			public class MonthlyChart : PltFileGeneratorBase, IPlugin, IMonthlyChart
			{
				...
				*/
				...
				public override async Task GenerateAsync(StreamWriter writer, DateTime hour)
				{
					...
					var png_format = this.UseCairo ? "pngcairo" : "png medium";
					await writer.WriteLineAsync($"set terminal {png_format} size {this.Width},{this.Height}");
					await writer.WriteLineAsync(string.Format("set output '{0}'",
							Path.Combine(
					...
					await writer.WriteLineAsync("set datafile separator ','");
					await writer.WriteLineAsync("set key outside");
					// もともとはこんな感じだった．何がどう作用しているのかは全くわからん．
					await writer.WriteLineAsync("set key outside Right box linetype -2 linewidth 1 samplen 4 spacing 1 width 3 height -2 autotitles");

					// 横軸
					await writer.WriteLineAsync("set xlabel 'Time'");
					await writer.WriteLineAsync("set xrange [ 1 : 24 ]");
					await writer.WriteLineAsync("set xtics border mirror norotate 1,1,24");

					// 縦軸
					await writer.WriteLineAsync("set ylabel 'Electric Power [kWh]'");

					var range = this.Maximum - this.Minimum;
					if (range > 0)
					{
						await writer.WriteLineAsync($"set yrange [{Minimum} : {Maximum}] noreverse nowriteback");
						int span = 100;
						int mytics = 5;
						if (range < 200)
						{
						...
						await writer.WriteLineAsync($"set ytics border mirror norotate {Minimum},{span},{Maximum}");

						// (1.3.13.1)補助目盛(主目盛りを何等分するかということ)．
						// とりあえず常に出力する設定にしておく．
						await writer.WriteLineAsync($"set mytics {mytics}");
					}
					...
					{
						await writer.WriteLineAsync("set y2tics border mirror norotate (\"\" {BorderLine.Value})");
						await writer.WriteLineAsync("set grid y2tics noxtics");
					}
					...
					await writer.WriteLineAsync("plot " + string.Join(", ", source_list.ToArray()));
				...
				public async Task<DateTime> UpdateAsync(DateTime lastUpdate)
				{
					...
					{
						await this.DrawChartAsync(latest_hour);
						return latest_hour;
						// 理想的にはこれだけの話．
					}
					...
					else
					{
						...
						while (lastUpdate.Month != latest_hour.Month || lastUpdate.Year != latest_hour.Year)
						{
							...
							await DrawChartAsync(next);  // A
							lastUpdate = next;
						}

						if (latest_hour.Day != 1 || latest_hour.Hour != 0)
						{
							await DrawChartAsync(latest_hour); // B
						}
						return latest_hour;
					}
				}
				...
				// (1.3.11)
				protected async Task DrawChartAsync(DateTime time)
				{
					// tempファイルのwriterをつくってGeneratePlt(writer, time)してgnuplot実行！！
					// ...ということを，このメソッド1つでやってくれるんだった．
					await Gnuplot.GenerateChartAsync(this, time);
				}

				#region テスト用メソッド

				// (1.3.8) 試しに出力してみるメソッド．
				public async Task OtameshiPltAsync(DateTime date, string destination)
				{
					//this.CurrentDate = date;
					using (var writer = new StreamWriter(File.Open(destination, FileMode.Create, FileAccess.Write)))
					{
						await this.GenerateAsync(writer, new DateTime(date.Year, date.Month, date.Day, date.Hour, 0, 0));
					}
				}
				...
				public async Task DrawChartTestAsync(DateTime time)
				{
					var hour = new DateTime(time.Year, time.Month, time.Day, time.Hour, 0, 0);
					await DrawChartAsync(hour);
				}
				#endregion
			...
			public interface IMonthlyChart
			{
				...
				Task OtameshiPltAsync(DateTime date, string destination);
				Task DrawChartTestAsync(DateTime time);
			}
