	...
	namespace Legacy
	{
		...
		namespace New
		{
			...
			public class DailyCsvGenerator : ConsumptionData, IPlugin, IDailyCsvGenerator
			{
				...
				public DailyCsvGenerator(Data.IConnectionProfile profile) : base(profile)
				{ }
				...
				public async Task<DateTime> UpdateFilesAsync(DateTime latestData, int recursiveCount = 0)
				{
					...
					try
					{
						...
						{
							// あれば，そのファイルを開く．
							csv_last_data = await this.RetrieveLatestTimeAsync(target, date_origin);
						...
						else
						{
							...
							{
								csv_last_data = await UpdateFilesAsync(date_origin, recursiveCount + 1);
							...
							await OutputHeaderAsync(writer, date_origin);
						...
						var data = await this.GetParticularConsumptionsAsync(csv_last_data, date_end, all_channels);

						csv_last_data = await RecordAsync(writer, csv_last_data, data);
					}
				...
				public async Task<bool> OutputOneDayAsync(DateTime target_date)
				{
					// dateは時刻を度外視する．
					...
					return date_end == (await UpdateFilesAsync(date_end, RECURSIVE_COUNT_LIMIT));

				}
				#endregion


				protected virtual async Task<DateTime> RecordAsync(StreamWriter writer, DateTime csv_last_data, IDictionary<DateTime, IDictionary<int, int>> data)
				{
					foreach (var onetime in data.OrderBy(d => d.Key))
					{
						// onetime.Value : { 1 => 36, 2 => 39, 3 => 4 }
						await OutputDataRowAsync(writer, onetime.Key, onetime.Value);
				...
				// (1.3.6)
				async Task<DateTime> RetrieveLatestTimeAsync(string fileName, DateTime dateOrigin)
				{
				...
				public void Configure(System.Xml.Linq.XElement config)
				{
					...
					this.UpdateAction = async (time) => await UpdateFilesAsync(time);
				}
				#endregion
				...
				public async Task OutputHeaderAsync(StreamWriter writer, DateTime date)
				{
					// ※↓これを動的に生成したい(日付の書式？)．
					await writer.WriteLineAsync("# " + date.ToString("MM/dd/yyyy"));
					string[] series = Columns.Select(c => c.Name).ToArray();
					await writer.WriteLineAsync("# 時刻," + string.Join(",", series));

				}
				#endregion

				#region *データ行を出力(OutputDataRow)
				public async Task OutputDataRowAsync(StreamWriter writer, DateTime time, IDictionary<int, int> data)
				{
					// ※決め打ちバージョン．
					...
					await writer.WriteLineAsync($"{time.ToString(time_format)},{string.Join(",", output_data)}");
				}
				#endregion
			...
			{
				Task<DateTime> UpdateFilesAsync(DateTime latestData, int recursiveCount = 0);
				Task<bool> OutputOneDayAsync(DateTime target_date);
			...
			{
				public DailyHourlyCsvGenerator(Data.IConnectionProfile profile) : base(profile)
				{ TimeFormat = "HH"; }

				protected override async Task<DateTime> RecordAsync(StreamWriter writer, DateTime csv_last_data, IDictionary<DateTime, IDictionary<int, int>> data)
				{
					while (true)
					{
						...
						if (next_hour_data.Count() == 6)
						{
							...
							// onetime.Value : { 1 => 36, 2 => 39, 3 => 4 }
							await OutputDataRowAsync(writer, next_hour, hourly_total);
